...
-------
  ...
  - sattler  : top-down gradient-cosine bipartition, Sattler TNNLS 2021
  - fedsoft  : softmax(mean similarity), Ruan & Joe-Wong AAAI 2022 (simplified)
  - hflts    : HFLTS envelope -> defuzzify, this prototype's Gap A

Reports both per-client test accuracy AND the Adjusted Rand Index of
argmax(pi) against the cohort's ground-truth primary cluster label,
so we can tell apart "the model is good" from "the soft assignment
recovered the cluster structure".
...
def evaluate(cohort, method: str, n_rounds: int, seed: int) -> dict:
    ...
    pi = trainer.state.pi
    pred_cluster = pi.argmax(axis=1)
    ari = float(adjusted_rand_score(cohort.primary_label, pred_cluster))
    return {
...
def format_markdown(results: dict, n_seeds: int, dataset: str) -> str:
    ...
    for name, m in results.items():
        rows.append(
            f"| {name} | {cell(m['mean_acc'])} | {cell(m['boundary_acc'])} | {cell(m['ari'])} |"
        )
    return "\n".join(rows) + f"\n\n_aggregated over {n_seeds} seeds_"


def run_dataset(dataset: str, n_seeds: int, n_rounds: int) -> dict:
    out = {m: {"mean_acc": [], "boundary_acc": [], "ari": []} for m in METHODS}
